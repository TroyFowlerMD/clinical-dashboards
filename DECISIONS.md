...
Consequences: IT Mode requires a protected hostname or reverse proxy in Troy's non-COCM Cloudflare account. The Access-protected Worker hostname automatically enters IT Mode after authentication, even if the authentication redirect drops the optional `it_mode=1` query parameter. The public GitHub Pages hostname still requires a protected redirect and never creates contextual controls by itself.
