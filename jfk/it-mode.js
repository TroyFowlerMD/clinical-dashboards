(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = [
      '.it-mode-entry{display:inline-flex;margin-top:6px;padding:3px 7px;border:1px solid var(--border);border-radius:6px;background:transparent;color:var(--text-muted);font:600 10px/1.3 var(--font-display,inherit);cursor:pointer}.it-mode-entry:hover{border-color:var(--teal);color:var(--teal)}',
  ...
  function showConfirmation(count) { var dialog = document.getElementById('it-confirm-dialog'); if (!dialog) { dialog = document.createElement('dialog'); dialog.id = 'it-confirm-dialog'; dialog.className = 'it-confirm-dialog'; dialog.innerHTML = '<h2>IT requests sent</h2><p><span class="it-confirm-count"></span> submitted successfully.</p><p>Copy the task prompt below and paste it into Codex or ChatGPT Work to complete the requested changes.</p><div><button type="button" class="it-copy-prompt">Copy task prompt</button><button type="button" class="it-close-confirm">Close</button></div>'; dialog.querySelector('.it-copy-prompt').addEventListener('click', function () { navigator.clipboard.writeText(CODEX_PROMPT).then(function () { dialog.querySelector('.it-copy-prompt').textContent = 'Copied'; }); }); dialog.querySelector('.it-close-confirm').addEventListener('click', function () { dialog.close(); }); document.body.appendChild(dialog); } dialog.querySelector('.it-confirm-count').textContent = count + (count === 1 ? ' request was' : ' requests were'); dialog.showModal(); }
